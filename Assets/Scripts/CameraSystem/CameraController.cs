...
namespace CameraSystem
{
    public class CameraController : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            EventCenter<GameEvent>.Instance.AddListener<Vector3>(GameEvent.CameraMoveToPosition, CameraMoveToPosition);
        }
        
        private void OnDestroy()
        {
            if (EventCenter<GameEvent>.IsInstanceNull) return;
            EventCenter<GameEvent>.Instance.RemoveListener<Vector3>(GameEvent.CameraMoveToPosition, CameraMoveToPosition);
        }
        
        private void CameraMoveToPosition(Vector3 args)
        {
            var targetPos = new Vector3(args.x, args.y, _cameraTrans.position.z);
            _cameraTrans.DOMove(targetPos, 0.5f);
        }
